...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="apppack/list" method="post" name="Form" id="Form">
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								<td class='center' style="width: 30px;margin-top: 10px;">
									<div style="margin-top: 10px;"></div><label ><input type='checkbox' name='ids' value="${var.id}" /><span class="lbl"></span></label></div>
								</td>
								
								<td class='center' style="width: 30px;"><div style="margin-top: 16px;">${vs.index+1}</div></td>
								<td ><div style="margin-top: 16px;">${var.miaosu}</div></td>
										<td style="height: 50px;">
										
										<a href="<%=basePath%>uploadFiles/uploadImgs/${var.path}"  class="bwGal"><img src="<%=basePath%>uploadFiles/uploadImgs/${var.path}"  style="max-height: 60px;width: 100px"></a>
										</td>
								<td style="width: 60px;" class="center">
										
										<div style="margin-top: 16px;">	
										<a style="cursor:pointer;text-align: center;" title="删除" onclick="del('${var.id}','${var.path}');" class="tooltip-error" data-rel="tooltip" title="" data-placement="left"><span class="red" style=""><i class="icon-trash"></i> </span> </a>
...
<script type="text/javascript">
	...
	function add(){
		 ...
		 diag.Drag=true;
		 diag.Title ="新增";
		 diag.URL = '<%=basePath%>apppack/goAdd';
		 diag.Width = 800;
		 diag.Height = 490;
	...
	function newedit(id){
		 ...
		 diag.Drag=true;
		 diag.Title ="修改";
		 diag.URL = '<%=basePath%>apppack/go_Edit?appid='+id;
		 diag.Width = 800;
		 diag.Height = 490;
		 diag.CancelEvent = function(){ //关闭事件
			 if('${page.currentPage}' == '0'){
	...
	function del(Id,path){
		bootbox.confirm("确定要删除吗?", function(result) {
			if(result) {
				var url = "<%=basePath%>apppack/delete?apppack_id="+Id+"&path="+path+"&tm="+new Date().getTime()+"&currentPage="+$("#nowpage").val();
				$.ajax({
