...
<body>
	<form  action="auth/add" name="menuForm" id="menuForm" method="post" >
				...
				<td>
					<select name="PARENT_ID" id="parentId" class="input_txt" onchange="setMUR()"  title="菜单">
						...
						<c:forEach items="${menuList}" var="auth">
							<option value="${auth.id }" onclick="">${auth.authname }</option>
						</c:forEach>
					</select>
				</td>
			</tr>
			<tr>
				<td><input type="text" name="authname" id="authname" placeholder="这里输入菜单名称" value="" title="名称"/></td>
			</tr>
			<tr>
				<td><input type="text" name="authpath" id="authpath" placeholder="这里输入链接地址" value="" title="地址"/></td>
			</tr>
			<tr>
				<td><input type="number" name="authorder" id="authorder" placeholder="这里输入序号" value="" title="序号"/></td>
			</tr>
			<tr>
