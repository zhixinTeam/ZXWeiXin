...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="apppicture/list" method="post" name="Form" id="Form">
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td class='center' style="width: 30px;">${vs.index+1}</td>
										<td>
										<a href="<%=basePath%>uploadFiles/uploadImgs/${var.path}"  class="bwGal"><img src="<%=basePath%>uploadFiles/uploadImgs/${var.path}"  width="100"></a>
										</td>
										<td>${var.createdate}</td>
								<td style="width: 30px;" class="center">
										
										<div>	<li><a style="cursor:pointer;" title="删除" onclick="del('${var.id}','${var.path}');" class="tooltip-error" data-rel="tooltip" title="" data-placement="left"><span class="red"><i class="icon-trash"></i></span> </a></li>
										</div>
								</td>
							</tr>
...
<script type="text/javascript">
	...
	function add(){
		 ...
		 diag.Drag=true;
		 diag.Title ="新增";
		 diag.URL = '<%=basePath%>apppicture/goAdd';
		 diag.Width = 800;
		 diag.Height = 490;
	...
	function del(Id,path){
		bootbox.confirm("确定要删除吗?", function(result) {
			if(result) {
				var url = "<%=basePath%>apppicture/delete?picture_id="+Id+"&path="+path+"&tm="+new Date().getTime()+"&currentPage="+$("#nowpage").val();
				$.ajax({
