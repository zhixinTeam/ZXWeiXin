		 ...
		 function sendmsg(){
			 ...
			 if(msg_type=="text_msg"){
				 ...
				 $.ajax({
					...
					/*  var content_text =$('#content_text').val();
					 var msgtype =$('#msgtype').val(); */
					 alert(recive_openIDs);
					 formData.append( "recive_openIDs", recive_openIDs ); 
				    $.ajax({
...
<div id="page-content" class="clearfix">
  ...
  <br>
   ...
   <form action="tool/sendmsg" name="msgForm" id="msgForm" method="post"   >
		  ...
		  <input type="hidden" name="mse" id="ms" value="hi big bug "/>
		  </div>
		...
		function add_imgmsg(){
			 var start_div="<div class='img' id='img_div"+index+"'><div class='row'>"+
			  
			 " <div class='col-xs-4'>"+
			    "  <label for='name'>标题</label>"+
			      
			   " <input type='text'  id ='title'  name='title"+index+"'  class='form-control' placeholder='请输入标题'>"+
			 " </div>"+
		...
		"	    <input id='tp"+index+"' name='tp"+index+"' type=\"file\" class=\"file\" >"+
		"	  </div>"+
		"	</div>"+
