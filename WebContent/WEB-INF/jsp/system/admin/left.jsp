			...
			<c:forEach items="${authList}" var="menu">
				<li id="lm${menu.id }">
					  <a style="cursor:pointer;" class="dropdown-toggle" >
						<i class="${menu.iconcls == null ? 'icon-desktop' : menu.iconcls}"></i>
					   ...
					   <ul class="submenu">
							<c:forEach items="${menu.children}" var="sub">
								<c:choose>
									<c:when test="${not empty sub.authpath}">
									<li id="z${sub.id }">
									<a style="cursor:pointer;" target="mainFrame"  onclick="siMenu('z${sub.id }','lm${menu.id }','${sub.authname }','${sub.authpath }')"><i class="icon-double-angle-right"></i>${sub.authname }</a></li>
									</c:when>
									 <c:otherwise>
									<li><a href="javascript:void(0);"><i class="icon-double-angle-right"></i>${sub.authname }</a></li>
									</c:otherwise> 
								</c:choose>
							</c:forEach>
				  		</ul> 
