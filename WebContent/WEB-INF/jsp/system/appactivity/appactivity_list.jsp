...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="appactivity/list" method="post" name="Form" id="Form">
				...
				<c:choose>
					<c:when test="${not empty varList}">
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								<td class='center' style="width: 30px;margin-top: 10px;">
									<div style="margin-top: 10px;"><label><input type='checkbox' name='ids' value="${var.id}" /><span class="lbl"></span></label></div>
								</td>
								
								<td class='center' style="width: 30px;"><div style="margin-top: 16px;">${vs.index+1}</div></td>
								<td><div style="margin-top: 16px;">${var.title}</div></td>
										<td height="50px">
										
										<a href="<%=basePath%>uploadFiles/uploadImgs/${var.path}"  class="bwGal"><img src="<%=basePath%>uploadFiles/uploadImgs/${var.path}"  style="max-height: 60px;" width="100"></a>
										</td>
										<td ><div style="margin-top: 16px;">${var.createdate}</div></td>
								<td style="width: 60px;" class="center">
										
										<div style="margin-top: 16px;"> <a style="cursor:pointer;" title="删除" onclick="del('${var.id}','${var.path}');" class="tooltip-error" data-rel="tooltip" title="" data-placement="left"><span class="red"><i class="icon-trash"></i></span> </a>
										&nbsp;<a style="cursor:pointer;" title="修改" onclick="newedit('${var.id}');" class="tooltip-error" data-rel="tooltip" title="" data-placement="left"><span class="green" ><i class="icon-edit" ></i></span> </a>
										</div>
								</td>
							</tr>
...
<script type="text/javascript">
	...
	function add(){
		 ...
		 diag.Drag=true;
		 diag.Title ="新增";
		 diag.URL = '<%=basePath%>appactivity/goAdd';
		 diag.Width= 800;
		 diag.Height = 490;
	...
	function newedit(id){
		 ...
		 diag.Drag=true;
		 diag.Title ="修改";
		 diag.URL = '<%=basePath%>appactivity/go_Edit?appid='+id;
		 diag.Width = 800;
		 diag.Height = 490;
		 diag.CancelEvent = function(){ //关闭事件
			 if('${page.currentPage}' == '0'){
	...
	function del(Id,path){
		bootbox.confirm("确定要删除吗?", function(result) {
			if(result) {
				var url = "<%=basePath%>appactivity/delete?appactivity_id="+Id+"&path="+path+"&tm="+new Date().getTime()+"&currentPage="+$("#nowpage").val();
				$.ajax({
